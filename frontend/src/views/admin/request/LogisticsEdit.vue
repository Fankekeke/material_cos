...
export default {
  ...
  methods: {
    selectLogistics (orderId) {
      this.$get(`/cos/request-supply-info/detail/${orderId}`).then((r) => {
    ...
    setFormValues ({...logistics}) {
      ...
      this.selectLogistics(logistics.id)
    },
    reset () {
      this.loading = false
    ...
    receipt () {
      this.$get(`/cos/request-supply-info/rurchasePut/${this.rowId}`).then((r) => {
        this.$emit('success')
      })
    },
    handleSubmit () {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          this.$put('/cos/request-supply-info', {
            'id': this.rowId,
